        ...
        if st.button("📂 Load Saved Roster"):
            try:
                ...
                if not df_cloud.empty and owner_name in df_cloud['Manager'].values:
                    ...
                    # Convert saved raw names back to display names for the UI
                    saved_raw_names = saved_str.split(", ")
                    # Filter to make sure players still exist
                    ...
                    # No rerun needed here if we assign session_state correctly
                else:
                    st.toast("No saved team found.", icon="⚠️")
            except:
                st.error("Connection failed.")

# --- 3. RESERVE SPACE FOR DASHBOARD ---
# We create an empty container here. We will fill it LATER, after we know what you clicked.
dashboard_placeholder = st.container()

st.divider()

# --- 4. RENDER TABLES (Bottom of screen logic happens first) ---
def render_position_table(position_name, header_text):
    pos_df = all_players[all_players['position'] == position_name].copy()
    
    # Check boxes based on session state
    pos_df['Draft'] = pos_df['display_name'].isin(st.session_state['my_roster'])
    
    # Sort strictly by points to prevent "Dancing Rows"
    ...
    edited_df = st.data_editor(
        ...
        column_config={
            ...
            "projected_points": st.column_config.NumberColumn("Pts", format="%.1f", width="small")
        },
...
# Capture the FRESH click data immediately
current_selection = sel_qb + sel_rb + sel_wr + sel_te + sel_k + sel_def
st.session_state['my_roster'] = current_selection

# --- 5. FILL THE DASHBOARD (Now that we have the fresh data) ---
# We jump back up to the top container we reserved earlier
with dashboard_placeholder:
    
    # Get the data for the selected players
    my_team_data = all_players[all_players['display_name'].isin(current_selection)]
    ...
    with d3:
        ...
        if valid_roster:
            # We use a key to ensure the button doesn't get stuck in an old state
            if st.button("💾 Submit Roster", type="primary", use_container_width=True, key="save_btn"):
                with st.spinner("Saving..."):
                    sheet = get_sheet()
                    if sheet:
                        # Save RAW names for cleaner DB
            ...
            st.button("Roster Invalid", disabled=True, use_container_width=True, key="invalid_btn")
