...
def load_players():
    ...
    # Standard format: "Patrick Mahomes (KC)"
            ...
            try:
                ...
                if not df_cloud.empty and owner_name in df_cloud['Manager'].values:
                    ...
                    if target_col in user_row and user_row[target_col]:
                        ...
                        # Restore roster using RAW names to find the players
...
def calculate_multipliers(manager, week_num):
    multipliers = {}
    default_mult = 1.0
    # Default everyone to 1.0
    for name in all_players['name']:
        multipliers[name] = default_mult
...
# Key Change: Multipliers are mapped to RAW names now to be safer
...
def render_position_table(position_name, header_text):
    ...
    # Check boxes based on RAW names in session state
    # (We store raw names in session_state now to avoid formatting mismatches)
    ...
    def format_name(row):
        base = row['display_name'] # e.g. "Jalen Hurts (PHI)"
        m = row['mult']
    ...
    # Create a new column specifically for the UI
    pos_df['ui_name'] = pos_df.apply(format_name, axis=1)
    
    # Calculate boosted points for sorting
    ...
    edited_df = st.data_editor(
        ...
        disabled=["ui_name", "boosted_points"],
        height=450
    )
    
    # Logic to return the RAW names of selected players
    # We have to match the 'ui_name' back to the raw 'name'
    selected_ui_names = edited_df[edited_df['Draft'] == True]['ui_name'].tolist()
    
    # Filter original df to find the raw names corresponding to these UI names
    # (This is why we needed the 'ui_name' in the pos_df)
    selected_raw_names = pos_df[pos_df['ui_name'].isin(selected_ui_names)]['name'].tolist()
